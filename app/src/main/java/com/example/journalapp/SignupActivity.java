            ...
            firebaseAuth.createUserWithEmailAndPassword(
                ...
                public void onComplete(@NonNull Task<AuthResult> task) {
                    if (task.isSuccessful()){
                        ...
                        db.collection("User").document(firebaseAuth.getCurrentUser().getUid().toString().trim()).set(data).addOnSuccessListener(new OnSuccessListener<Void>() {
                            @Override
                            public void onSuccess(Void unused) {
                                Toast.makeText(SignupActivity.this, "Succesfully added the user name", Toast.LENGTH_SHORT).show();
                            }
                        });
